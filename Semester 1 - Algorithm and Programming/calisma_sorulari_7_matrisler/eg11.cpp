...
int main() {
    ...
    // -------- Ask for input
    int chosen_major, student_i;
    printf("\nYeni ogrenci hangi bolume girecek: ");
    scanf("%d", &chosen_major);
    chosen_major--;
    
    // ---- Check the capacity
    if (num_students[chosen_major] < capacity) {
        printf("\nIstenilen sirayi giriniz: ");
        scanf("%d", &student_i);
        student_i--;
        
        for (i=num_students[chosen_major] ; i>=student_i ; i--) {  // slide the old students downwards
            for (j=0 ; j<num_class ; j++) {
                for (k=0 ; k<3 ; k++) {
                    database[chosen_major][i+1][j][k] = database[chosen_major][i][j][k];
                }
            }
        }
        
        for (i=mean=0 ; i<num_class ; i++) {
            database[chosen_major][student_i][i][0] = rand()%101;
            mean += 0.4*database[chosen_major][student_i][i][0];
            
            database[chosen_major][student_i][i][1] = rand()%101;
            mean += 0.6*database[chosen_major][student_i][i][1];
            
            database[chosen_major][student_i][i][2] = mean;
        }
        
        num_students[chosen_major]++;
    }
    else {
        printf("\nKapasite yetersiz.");
    }
